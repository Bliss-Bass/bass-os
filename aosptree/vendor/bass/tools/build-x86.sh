...
export USE_BLISS_RLT=false
# autogenerated clear exports go here
export USE_MDS_PLATFORM=false
export SET_FOR_RELEASE=false
export USE_DROID_VNC_PLATFORM=false
export BUILD_ONLY_EXTRA_PACKAGES=false
export USE_DROID_VNC=false
export INCLUDE_GENERIC_MDM=false
export ADD_VNCFLINGER=false
export AD_SIGNAGE=false
export BASS_ID_TITLE=""
export ADD_WATERMARK=false
export USE_DAIJISHOU=false
export USE_VAPOR_LAUNCHER=false
export USE_SCREENVIEW=false
export BLISS_PER_WINDOW_INPUT_ROTATION=false
export INCLUDE_VENDOR_INPUT=false
...
function displayHelp() {
    ...
    echo ""
    echo "Autogenerated Options:"
    # autogenerated help items go here
    echo "--mdsp          Include multidisplaysettings platform"
    echo "--release          Sets build up for release (requires shipper/shippy setup)"
    echo "--dvncp          Include DroidVNC-NG for platform"
    echo "--boep          Build only extra packages"
    echo "--dvnc          Add net.christianbeier.droidvnc_ng VNC server"
    echo "--genmdm          Include generic MDM from vendor MDM addon"
    echo "--vncflinger          Add VncFlinger and Configuration services"
    echo "--signage (name)         Include required configs for Ad/Signage builds"
    ...
    echo "--pwir          use per window input rotation"
    echo "--vndinput          use generated vendor input definition"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --mdsp)
               USE_MDS_PLATFORM=true
               shift
               ;;
        --release)
               SET_FOR_RELEASE=true
               shift
               ;;
        --dvncp)
               USE_DROID_VNC_PLATFORM=true
               shift
               ;;
        --boep)
               BUILD_ONLY_EXTRA_PACKAGES=true
               shift
               ;;
        --dvnc)
               USE_DROID_VNC=true
        ...
        --sview)
               ...
               ;;
        --pwir)
               BLISS_PER_WINDOW_INPUT_ROTATION=true
               shift
               ;;
        --vndinput)
               INCLUDE_VENDOR_INPUT=true
               shift
               ;;
        --useblur)
...
export USE_BLISS_RLT=${USE_BLISS_RLT:-false};
# autogenerated export items go here
export USE_MDS_PLATFORM=${USE_MDS_PLATFORM:-false};
export SET_FOR_RELEASE=${SET_FOR_RELEASE:-false};
export USE_DROID_VNC_PLATFORM=${USE_DROID_VNC_PLATFORM:-false};
export BUILD_ONLY_EXTRA_PACKAGES=${BUILD_ONLY_EXTRA_PACKAGES:-false};
export USE_DROID_VNC=${USE_DROID_VNC:-false};
export INCLUDE_GENERIC_MDM=${INCLUDE_GENERIC_MDM:-false};
export ADD_VNCFLINGER=${ADD_VNCFLINGER:-false};
export AD_SIGNAGE=${AD_SIGNAGE:-false};
export BASS_ID_TITLE=${BASS_ID_TITLE:-};
...
export BLISS_PER_WINDOW_INPUT_ROTATION=${BLISS_PER_WINDOW_INPUT_ROTATION:-false};
export INCLUDE_VENDOR_INPUT=${INCLUDE_VENDOR_INPUT:-false};
...
echo "Includemultidisplaysettingsplatform: ${USE_MDS_PLATFORM}";
echo "Setsbuildupforreleaserequiresshippershippysetup: ${SET_FOR_RELEASE}";
echo "IncludeDroidVNCNGforplatform: ${USE_DROID_VNC_PLATFORM}";
echo "Buildonlyextrapackages: ${BUILD_ONLY_EXTRA_PACKAGES}";
echo "AddnetchristianbeierdroidvncngVNCserver: ${USE_DROID_VNC}";
echo "IncludegenericMDMfromvendorMDMaddon: ${INCLUDE_GENERIC_MDM}";
echo "AddVncFlingerandConfigurationservices: ${ADD_VNCFLINGER}";
echo "IncluderequiredconfigsforAdSignagebuilds: ${AD_SIGNAGE} ID:${BASS_ID_TITLE}";
...
echo "Useperwindowinputrotation: ${BLISS_PER_WINDOW_INPUT_ROTATION}";
...
if [ "$BUILD_ONLY_EXTRA_PACKAGES" = true ]; then
    lunch bliss_x86_64-userdebug && make ${BUILD_EXTRA_PACKAGES} -j$jcores;
else
...
if [[ "$SET_FOR_RELEASE" == "true" ]]; then
    cd iso/$build_filename
    echo -e "Uploading OTA file to shipper server"
    shippy -y
    cd ../..
fi
